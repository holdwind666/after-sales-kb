...
function Resolve-PlaywrightCli {
    ...
    $cands = @((Join-Path $env:APPDATA "npm\playwright-cli.ps1"))
    foreach ($c in $cands) {
...
function Resolve-Config {
    ...
    $mainCfg = Join-Path $env:LOCALAPPDATA "AfterSalesSupport\config\settings.json"
    if (Test-Path -LiteralPath $mainCfg -PathType Leaf) {
        $main = Get-Content -LiteralPath $mainCfg -Encoding UTF8 -Raw | ConvertFrom-Json
        if ($main.cache_root) {
            $learningCfg = Join-Path ([string]$main.cache_root) "chatgpt_learning.json"
            if (Test-Path -LiteralPath $learningCfg -PathType Leaf) { return $learningCfg }
        }
    }
